...
router.get('/preferences', async (req, res) => {
    try {
        if (await data.userPreferences.checkUserPreferenceExists(req.session.userID)) {
            ...
            res.status(200).render('pages/userPreferencesForm', { title: "Tell Us What You Like", name: userName, partial: "preferences-scripts" });
        }
    } catch (e) {
...
router.post('/preferences', async (req, res) => {
    try {
        ...
        if (!userPref) {
            res.status(400).render("pages/somethingWentWrong", { title: "Something Went Wrong" });
        } else {
